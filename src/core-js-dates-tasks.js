...
function getDayName(date) {
  ...
  return months[new Date(date).getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  nextFriday.setUTCDate(nextFriday.getUTCDate() + add[current]);
  return nextFriday;
}
...
function getCountDaysInMonth(month, year) {
  ...
  const days = [31, isFull ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  return days[month - 1];
}
...
function formatDate(date) {
  ...
  const minutes = mydate.getUTCMinutes().toString().padStart(2, '0');
  const seconds = mydate.getUTCSeconds().toString().padStart(2, '0');
  const inDaytime = hours < 12 ? 'AM' : 'PM';

  hours %= 12;
  if (hours === 0) {
    hours = 12;
  }
  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${inDaytime}`;
}
/**
...
function getNextFridayThe13th(date) {
  ...
  while (count < maCheck) {
    const nextFriday = new Date(year, month, 13);
